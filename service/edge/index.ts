...
export class Service {
  ...
  private async connect(): Promise<WebSocket> {
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.on('close', (code, reason) => {
        ...
        DEBUG && console.info(`连接已关闭： ${reason} ${code}`)
      })

      ws.on('message', (message, isBinary) => {
        let pattern = /X-RequestId:(?<id>[a-z|0-9]*)/
        if (!isBinary) {
          DEBUG && console.debug('收到文本消息：%s', message)
          let data = message.toString()
          if (data.includes('Path:turn.start')) {
            // 开始传输
            let matches = data.match(pattern)
            let requestId = matches.groups.id
            DEBUG && console.debug(`开始传输：${requestId}……`)
            ...
            if (executor) {
              ...
              DEBUG && console.debug(`传输完成：${requestId}……`)
            } else {
              DEBUG && console.debug(`请求已被丢弃：${requestId}`)
            }
          }
        } else if (isBinary) {
      ...
      ws.on('error', (error) => {
        DEBUG && console.error(`连接失败： ${error}`)
        reject(`连接失败： ${error}`)
      })
      ws.on('ping', (data) => {
        DEBUG && console.debug('ping %s', data)
      })
      ws.on('pong', (data) => {
        DEBUG && console.debug('pong %s', data)
      })
    })
  }
  ...
  public async convert(ssml: string, format: string) {
    ...
    let result = new Promise((resolve, reject) => {
      ...
      DEBUG && console.info(`开始转换：${requestId}……`)
      DEBUG && console.debug(`准备发送配置请求：${requestId}\n`, configMessage)
      this.ws.send(configMessage, (configError) => {
        if (configError) {
          DEBUG && console.error(`配置请求发送失败：${requestId}\n`, configError)
        }
        ...
        let ssmlMessage =
          ...
          `Path:ssml\r\n\r\n` +
          ssml
        DEBUG && console.debug(`准备发送SSML消息：${requestId}\n`, ssmlMessage)
        this.ws.send(ssmlMessage, (ssmlError) => {
          if (ssmlError) {
            DEBUG && console.error(`SSML消息发送失败：${requestId}\n`, ssmlError)
          }
        })
      })
    })
    ...
    // 收到请求，清除超时定时器
    if (this.timer) {
      DEBUG && console.debug('收到新的请求，清除超时定时器')
      clearTimeout(this.timer)
    }
    // 设置定时器，超过10秒没有收到请求，主动断开连接
    DEBUG && console.debug('创建新的超时定时器')
    this.timer = setTimeout(() => {
      if (this.ws && this.ws.readyState == WebSocket.OPEN) {
        DEBUG && console.debug('已经 10 秒没有请求，主动关闭连接')
        this.ws.close(1000)
        this.timer = null
      }
    }, 10000)
    ...
    DEBUG && console.info(`转换完成：${requestId}`)
    DEBUG && console.info(`剩余 ${this.executorMap.size} 个任务`)
    return data
  }
}
